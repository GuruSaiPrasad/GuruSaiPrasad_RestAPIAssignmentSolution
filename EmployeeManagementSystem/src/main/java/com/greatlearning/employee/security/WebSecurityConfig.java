...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		http.authorizeRequests().antMatchers("/login").permitAll()
				.antMatchers(HttpMethod.POST, "/api/user", "/api/role").permitAll()
				.antMatchers(HttpMethod.GET, "/api/employees/employeesList", "/api/employees/{employee_id}")
				.hasAnyAuthority("USER", "ADMIN")

				.antMatchers(HttpMethod.POST, "/api/employees/add").hasAuthority("ADMIN")
				...
				.hasAnyAuthority("USER", "ADMIN").anyRequest().authenticated().and().httpBasic().and().formLogin().and()
				.logout().logoutSuccessUrl("/login").permitAll().and().cors().and().csrf().disable().httpBasic();
	}
}
